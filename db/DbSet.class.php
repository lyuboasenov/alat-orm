...
use db\commands as commands;

class DbSet extends repository\Set {
   private $db;
   ...
   protected function saveObjectInternal(models\IModel $model) {
   ...
   protected function saveObjectReferencesInternal(models\IModel $model, $references) {
      ...
      if ($mappingType == MappingType::ForeignKey_ParentChild) {
         foreach($references as $ref) {
            commands\UpdateBuilder::table($refType)
         ...
         commands\UpdateBuilder::table($modelType)
         ...
         commands\DeleteBuilder::from($associationTable)
            ->where(mapper::getReferenceColumnName($modelType) . '=' . $model->id)
            ->build($this->db)->executeNonQuery();

         foreach($references as $ref) {
            commands\InsertBuilder::into($associationTable)
   ...
   protected function removeObjectInternal(models\IModel $model) {
      $mapper = Mapper::fromDomainModel($model);
